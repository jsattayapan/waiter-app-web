...
import {LanguageToggleButton, ZoneToggleButton} from './../components/toggleButton';

import { getTables, isTableOnHold, updateTableStatus } from './../brains/tables';
import { isAuth } from './../brains/authentication';
import { getCurrentOrder, createCustomerTable, getTableLogs } from './../brains/customerTable';
    ...
    isAuth(this.props.user.id, (data) => {
      if(data){
        getTables((data) => {
          ...
          this.props.dispatch(setSectionTables(
            getTablesBySection(this.props.tables.allTables, this.props.tables.allTables[0].section)
          ));
        });
      }else{
          ...
          <div className='registerTablePopupContent'>
            <div className="container">
              <div className="row ">
                ...
                {this.state.error && <span style={{margin: '10px', color:'red', paddingTop:'5px'}}>กรุณาเลือก Zone และ ภาษา</span>}
              </div>
              ...
              <div className="row">
                <ZoneToggleButton selected={this.state.zone === 'A1'} label="A1" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B1'} label="B1" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B2'} label="B2" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B3'} label="B3" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B4'} label="B4" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B5'} label="B5" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B6'} label="B6" setZone={this.setZone}/>
                <ZoneToggleButton selected={this.state.zone === 'B7'} label="B7" setZone={this.setZone}/>
              </div>
              <div className="row">
                <h4 style={{margin: '10px'}}>จำนวนลูกค้า:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <span><button className="btn btn-success" onClick={() => this.decreaseGuest()}>-</button></span>
                  &nbsp; {this.state.number_of_guest} &nbsp;
                  <span><button className="btn btn-success" onClick={() => this.increaseGuest()}>+</button></span>
                </h4>
              </div>
              <div className="row">
              ...
              <div className="row">
                <LanguageToggleButton selected={this.state.language === 'ไทย'} label="ไทย" setLanguage={this.setLanguage}/>
                <LanguageToggleButton selected={this.state.language === 'อังกฤษ'} label="อังกฤษ" setLanguage={this.setLanguage}/>
              </div>
              <TopBuffer/>
              <div className="row">
                <div className="col-sm-6 text-center">
                  <button className="btn btn-danger" onClick={() => this.props.togglePopup()}>ยกเลิก</button>
                </div>
                <div className="col-sm-6 text-center">
                  <button className="btn btn-success" onClick={this.handleSubmit}>เปิดโต๊ะ</button>
                </div>
              </div>
            </div>
          </div>
